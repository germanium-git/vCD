...
for vm in compl_VMs:
    ...
    pprint(compl_VMs[vm])
    """
    cprint('\nUpdating the VM {}'.format(vm), 'yellow')
...
# Let it wait for a few seconds until the last VM is updated
time.sleep(5)
...
pprint(VMs_w_ip_afterswithover)
print('\n')

# Identify the longest VM name
vm_col_width = max(len(word) for row in VMs_w_ip_afterswithover.keys() for word in row) + 2  # padding
VM_comparison_lst = [['VM name', 'old IP', 'old network', 'new IP', 'new network']]
VM_comparison_dir = {}
# Populate list and directory
for vm in compl_VMs:
    VM_comparison_lst.append([ vm,
                           VMs_w_ip[vm]['adapters']['0']['ip_addr'],
                           VMs_w_ip[vm]['adapters']['0']['org_nw'],
                           VMs_w_ip_afterswithover[vm]['adapters']['0']['ip_addr'],
                           VMs_w_ip_afterswithover[vm]['adapters']['0']['org_nw']
                           ])
    VM_comparison_dir[vm] = {vm: {'old_ip': VMs_w_ip[vm]['adapters']['0']['ip_addr'],
                                  'old_nw': VMs_w_ip[vm]['adapters']['0']['org_nw'],
                                  'new_ip': VMs_w_ip_afterswithover[vm]['adapters']['0']['ip_addr'],
                                  'new_nw': VMs_w_ip_afterswithover[vm]['adapters']['0']['org_nw']
                                  }
                             }

col_width = max(len(word) for row in VM_comparison_lst for word in row) + 2  # padding
for row in VM_comparison_lst:
